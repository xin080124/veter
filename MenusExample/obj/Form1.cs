...
namespace MenusExample
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		{
			//bool saveOk = true;
			//then set the default name to the name of any opened file
			...
			if (saveFileDialog1.ShowDialog() == DialogResult.OK)
			{
				...
				//{
/*					if !(MessageBox.Show(this, "Overwrite existing file with the same name?", 
						"Saving file",MessageBoxButtons.YesNo) 
						== DialogResult.Yes)
					{
						saveOk = false;
					}
					we do not actually need the code above to check if a file exists
					so that we do not write over it. 
					The save dialog box will automatically
					generate a similar message if the property 
					.OverWritePrompt is set to true.
*/	//				if saveOK
	//				{
